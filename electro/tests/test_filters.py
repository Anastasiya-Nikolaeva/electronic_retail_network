...
class NetworkNodeFilterTests(APITestCase):
    def setUp(self):
        ...
        product1 = Product.objects.create(
            name="Продукт 1", model="Модель 1", release_date="2025-01-01"
        )
        product2 = Product.objects.create(
            name="Продукт 2", model="Модель 2", release_date="2025-02-01"
        )
        product3 = Product.objects.create(
            name="Продукт 3", model="Модель 3", release_date="2025-03-01"
        )
    ...
    def test_filter_by_country(self):
        ...
        response = self.client.get(
            url, {"country": "Россия"}, HTTP_AUTHORIZATION=f"Bearer {self.token}"
        )
    ...
    def test_filter_by_city(self):
        url = reverse("electro:networknode-list")
        response = self.client.get(
            url, {"city": "Москва"}, HTTP_AUTHORIZATION=f"Bearer {self.token}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
